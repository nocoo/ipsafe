...
class IpSafe {
  ...
  async testConnectivity(config) {
    return new Promise((resolve, reject) => {
      ...
      const options = {
        ...
        headers: {
          'User-Agent': config.userAgent || 'ipsafe/1.0.3',
          ...config.headers
        }
      };
  ...
  executeCommand(command, config = {}) {
    return new Promise((resolve, reject) => {
      // Parse command into program and arguments, preserving quoted strings
      const args = this.parseCommand(command);
      const program = args.shift();
      
      // Check if command needs full TTY access (interactive commands)
      const isInteractiveCommand = ['claude', 'vim', 'nano', 'less', 'more', 'top', 'htop'].includes(program.toLowerCase());
      
      const child = spawn(program, args, {
        stdio: isInteractiveCommand ? ['inherit', 'inherit', 'inherit'] : ['inherit', 'pipe', 'pipe']
      });
      ...
      if (!isInteractiveCommand) {
        // Stream stdout in real-time for non-interactive commands
        ...
        // Stream stderr in real-time for non-interactive commands
